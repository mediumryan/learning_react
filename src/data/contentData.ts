...
# ID = mediumryan, 레포지토리명 = learning_react_todo 인 경우
...
> 💡 **Tip** ： **git branch -M main** は、 **GitHub** の最新標準に合わせるためにローカルのブランチ名を統一させる重要なコマンドです。`,
  },
  {
    id: 'deploy-gh-pages-action',
    section: 9,
...
今回の講義では、 アプリのデプロイのために **gh-pages** というライブラリを使用します。
...
> 私たちが **npm run deploy** を実行すると、React（正確には npm）は自動的に **predeploy** スクリプトを先に実行します。
> つまり、個別にコマンドを入力しなくても、最新のコードをビルド（distフォルダを生成）してからそのままデプロイまで行ってくれる仕組みになっているのです。
...
> 💡 ただし、**GitHub**のサーバーに実際に反映されるまでには、およそ **1〜5分** ほどかかる場合があります。すぐにアクセスできない場合でも、少し時間をおいてから再度確認してみてください。世界中どこからでもアクセスできる、あなただけのアプリがまもなく誕生します。 🎉

### デプロイされたアプリにアクセスする
デプロイが完了すると、リモートリポジトリに「Deployment」というタブが新しく追加されていることを確認できます。
![github-deployment-1](/assets/images/contents/github_deployment_1.png)
そこに表示されている **GitHub Pages** が生成した公式URLをコピーし、ブラウザに貼り付けてみましょう！これで、あなたのTodoアプリは世界に公開され、他の人にも見てもらえる準備が整いました！🎉
![github-deployment-2](/assets/images/contents/github_deployment_2.png)`,
  },
  {
    id: 'deploy-final-summary',
