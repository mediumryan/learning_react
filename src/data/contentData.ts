...
리액트는 **SPA(Single Page Application)** 방식으로 동작합니다. 화면 전체가 '깜빡'이지 않고도 내용이 부드럽게 바뀌는 비밀이 바로 여기에 있습니다.
...
**1. 껍데기는 그대로, 알맹이만 교체**
**SPA**는 처음에 빈 도화지 같은 **index.html** 하나만 받아옵니다. 그 후 사용자가 메뉴를 클릭하면 페이지를 새로 받는 대신, 자바스크립트가 기존 화면의 부품을 떼어내고 **새로운 부품(컴포넌트)만** 그 자리에 꽂아 넣습니다.

**2. 주소창의 속임수(Routing)**
분명 페이지는 하나인데 주소창의 URL은 바뀝니다. 이는 실제 페이지가 이동한 것이 아니라, 자바스크립트가 주소창을 감시하다가 주소에 맞는 **데이터와 화면**을 즉석에서 그려주는 것입니다.

---

### 🚀 왜 더 부드럽고 빠를까요?

- **중복 로드 제거** : 로고, 메뉴 바, 배경 등 변하지 않는 부분은 그대로 두고 **바뀌어야 할 부분만** 건드리기 때문입니다.
- **데이터 중심 통신** : 무거운 HTML 뭉치를 매번 받는 게 아니라, 아주 가벼운 **데이터(JSON)** 만 주고받으므로 통신 속도가 압도적입니다.
...
- **버전 확인** : 터미널(또는 CMD)을 열고 아래와 같이 입력하세요. \`node -v\` 
- **권장 버전** : **Vite**를 사용하려면 **Node.js 18.0.0** 이상의 버전이 필요합니다.(안정적인 학습을 위해 최신 **LTS**버전 설치를 권장합니다.)
- **설치 방법** : 만약 명령어가 인식되지 않는다면 [Node.js 공식 홈페이지](https://nodejs.org/)에서 설치를 진행해 주세요.
...
4️⃣ **불필요한 코드 및 파일 삭제** 
> 1. **index.css** : 파일 내부의 모든 코드를 선택해서 삭제(비우기)하세요.
> 2. **App.css** : 이 파일은 사용하지 않으므로 **파일 자체를 삭제**합니다.
> 3. **App.jsx** : 아래 코드만 남기고 모두 지워주세요.
\`\`\`jsx
function App(){
...
위의 두 태그와 같이 HTML에서 닫지 않던 태그들도 JSX에서는 반드시 
\`\`\`jsx
<img />
<input />
\`\`\`
와 같이 **Self-closing** 하거나 닫아주어야 합니다.`,
    },
    {
      id: "section2-quiz-1",
      section: 2,
...
- JSX는 반드시 **하나의 부모태그**로 감싸야 하며, **className**을 사용한다.
...
이 도구는 리액트 앱에서 가장 기본적이면서도 강력한 기능을 담당하죠.
...
### ⚖️ 왜 두 개가 같이 나올까요?
리액트는 데이터만 주는 것이 아니라, 그 데이터를 **안전하게 바꿀 수 있는 전용 열쇠(변경함수)** 를 함께 줍니다. 
직접 데이터를 수정하지 않고 이 열쇠를 통해서만 수정해야 리액트가 화면을 다시 그려줄 수 있기 때문입니다.`,
    },
    {
...
즉, 연속적으로 상태값을 변경해도 항상 최신값을 참조해서 업데이트할 수 있게 해주는 것입니다.
...
> **로그 확인용이 아니라 '데이터의 신뢰성'을 위해** 사용한다는 점을 꼭 기억하세요.
`,
    },
    {
...
### 1️⃣ 단계: 훅 호출하기(준비)
가장 먼저 리액트에게 상태를 쓰겠다고 알리는**useState**를 불러와야 합니다. 훅은 반드시 컴포넌트의 **최상단**에서 호출해야 한다는 점을 잊지 마세요.
...
_브라우저에서 숫자가 실시간으로 바뀌나요? 
축하합니다! 이 카운터 앱이 여러분의 첫 번째 리액트 상태 관리 앱입니다! ✨_
...
축하합니다! 리액트에서 가장 중요한 개념 중 하나인 **State(상태)** 의 벽을 넘으셨습니다. 
이번 섹션에서 우리는 단순히 화면을 그리는 것을 넘어, 스스로 생각하고 변화하는 앱을 만드는 법을 배웠습니다.

---

### 📝 무엇을 배웠나요?

- **상태(State)의 개념** : 컴포넌트 내부에서 변할 수 있는 데이터를 기억하는 '기억 장치'임을 이해했습니다.
- **useState 훅** : 상태값을 안전하게 만들고, 이를 변경하기 위해 반드시 전용 함수인 **setter**를 사용한다는 규칙을 익혔습니다.
- **실습 - 카운터 앱** : 버튼 클릭 이벤트를 통해 상태를 업데이트하고, 숫자가 실시간으로 바뀌는 리액트의 반응성을 직접 체험했습니다.

---

### 🎁 다음 단계는 무엇인가요?

이제 우리 컴포넌트는 데이터를 기억할 줄 알게 되었습니다. 하지만 실제 앱은 수많은 컴포넌트가 서로 대화하며 데이터를 주고받습니다.

> _"내가 가진 **State**를 옆집 컴포넌트에게 전달하려면 어떻게 해야 할까요?"_

그 해답은 다음 섹션인 **Props**에 담겨 있습니다. 리액트의 또 다른 핵심 기둥을 배우러 가봅시다!`,
    },
    // Section 4: Props
    {
...
이때 컴포넌트끼리 서로 정보를 주고받아야 화면이 완성되는데, 그 통로 역할을 하는 것이 바로 **Props**입니다.
...
### 💡 함수를 왜 전달하나요?
리액트에서는 컴포넌트끼리 소통할 때 함수를 자주 주고받습니다. 특히 다음과 같은 상황에서 매우 유용합니다.
...
> **💡요약**
> **Props**는 무엇이든 담을 수 있는 만능 통로입니다. 그중에서도 부모의 **State**를 바꾸기 위한 **setter** 함수를 보내는 방식은 리액트 개발에서 가장 핵심적인 테크닉 중 하나입니다.`
},
    {
...
**Props**는 부모 컴포넌트로부터 전달받은 **읽기 전용(Read-Only)** 데이터입니다. 자식 컴포넌트가 임의로 이 값을 수정하려고 하면 에러가 발생하거나 리액트의 규칙이 깨지게 됩니다.
...
> **💡 값을 바꾸고 싶다면?**
> 직접 수정하는 대신, 이전 강의에서 배운 것처럼 부모가 보내준 **변경 함수(setter)** 를 호출하여 부모에게 "값을 바꿔주세요"라고 요청해야 합니다. 데이터의 원본을 가진 주인(부모) 만이 값을 고칠 수 있기 때문입니다.
...
### 3️⃣ 자료형에 따른 작성법 지키기
문자열을 제외한 모든 값(숫자, 변수, 함수, 객체 등) 은 반드시 중괄호 **{ }** 안에 작성해야 리액트가 올바른 데이터로 인식합니다.
...
### ✅ 핵심 요약
> 1. **Props** 는 부모가 자식에게 주는 **데이터** 다.
> 2. 자식은 Props를 **수정할 수 없다.**(읽기 전용)
> 3. **함수** 도 Props로 넘겨서 자식이 부모의 상태를 바꾸게 할 수 있다.
> 4. _**{ text }**_ 처럼 **구조 분해 할당** 을 쓰면 코드가 깔끔해진다.

---

부모로부터 함수를 전달받는 법까지 알게 되었으니, 이제 그 함수를 '언제' 실행할지 결정할 차례입니다. 
사용자의 클릭이나 입력에 반응하는 방법. 다음 섹션인 **이벤트(Event)** 에서 기다리고 있겠습니다. ⚡`,
    },
    // Section 5: Events
    {
...
- React 이벤트는 HTML 이벤트와 비슷하지만, **카멜 케이스(camelCase)** 를 사용합니다.
...
\`<button onClick={handleClick}>클릭</button>\`
 `,
  ...
  exp: 15,
  time: 5,
  content: `# 🆚 HTML vs React 이벤트 차이점

리액트의 이벤트 작성법은 HTML과 비슷해 보이지만, 딱 두 가지만 기억하면 됩니다. 바로 **이름**과 **전달 방식**입니다.

---

### 1️⃣ 표기법의 차이 (CamelCase)
HTML은 모든 이벤트를 소문자로 적지만, 리액트는 자바스크립트의 관례를 따라 **카멜 케이스(camelCase)** 로 작성해야 합니다.
...
HTML은 실행할 코드를 **"문자열"** 안에 담아 전달하지만, 리액트는 실행할 **함수 이름(참조)** 을 **{중괄호}** 안에 직접 넣습니다.
...
> \`onClick={handleClick()}\` 처럼 뒤에 괄호를 붙이면, 버튼을 누르기도 전에 함수가 즉시 실행되어 버립니다. 반드시 **이름**만 전달해야 한다는 점을 잊지 마세요.`
},
    {
...
### 📝 이벤트 처리용 함수 = 이벤트 핸들러
> 사용자가 버튼을 클릭하거나 글자를 입력하는 등의 사건을 **이벤트(Event)** 라고 합니다. 
그리고 이 사건이 일어났을 때, 리액트가 무엇을 해야 할지 알려주는 함수를 **이벤트 핸들러(Event Handler)** 라고 부릅니다. 말 그대로 발생한 사건을 **처리(Handle)** 하는 역할인 셈이죠.
...
### 💡 함수 이름 짓기
이벤트 핸들러의 이름은 아래와 같이 **어떤 동작을 하는지** 명확하게 짓는 것이 관습입니다.

\`\`\`jsx
handleClick // (클릭)
onChangeName // (이름 변경)
handleSubmit // (폼 제출) 
handleIncrease // (숫자 증가)
onMouseHover // (마우스 호버)
\`\`\`
 `,
    },
    {
  id: "event-with-arrow-function",
  section: 5,
  order: 3,
...
리액트에서 이벤트를 다룰 때 **화살표 함수(Arrow Function)** 는 매우 자주 사용되는 문법입니다.
코드를 더 **간결하게 작성할 수 있고**, 특히 이벤트에서 **인자를 전달**하거나 **간단한 로직을 작성**할 때 편리하게 사용할 수 있습니다.

![arrow function](/assets/images/contents/arrow_function.png)

---

### ❓ 이름 있는 함수 vs 화살표 함수, 무엇이 다른가요?

결론부터 말하면 **둘 다 결국 함수입니다.**

컴포넌트 상단에서 이름을 붙여 정의하든, 이벤트 위치에서 화살표 함수로 작성하든 **버튼에 실행할 함수를 연결하는 것**은 동일합니다.
...
#### 1️⃣ 인자가 필요한 함수를 실행할 때 (중요!)

특정 ID나 값을 함수에 전달해야 할 때, 아래처럼 작성하면 **클릭 시 실행이 아니라 즉시 실행**되어 버립니다.
...
이때 화살표 함수를 사용하면 **"클릭했을 때 실행할 함수"를 전달**할 수 있습니다.
...
> 즉, 화살표 함수는 **"지금 실행하는 것"이 아니라 "클릭 시 실행할 함수를 전달하는 역할"** 을 합니다.

---

#### 2️⃣ 하나의 이벤트에서 여러 작업을 처리할 때

별도의 함수를 만들지 않고도, 이벤트 위치에서 바로 여러 줄의 코드를 실행할 수 있습니다.
...
### 💡 요약: 상황에 맞는 선택이 중요
 ...
 - 코드가 길고 복잡하다면?
 👉 상단에 **이름 있는 함수**로 정리하는 것이 좋습니다.

- 인자를 전달하거나 짧은 로직이라면?
 👉 **화살표 함수**를 사용하면 간결하게 작성할 수 있습니다.

실제 리액트 코드에서는 이 두 가지 방식을 **상황에 맞게 함께 사용하는 경우가 많습니다.**
`
},
    {
...
이벤트의 진짜 목적은 사용자의 입력을 받아 **State**를 변경하는 것입니다.

그리고 여러분은 이미 **Section 3 - State 카운터 실습** 강의에서 클릭 이벤트를 한 번 경험해 보았습니다.

이번 강의에서는 그 기억을 되짚어보며, 이벤트가 발생했을 때 State가 어떻게 바뀌고, 그 결과로 화면이 어떻게 업데이트되는지 다시 한 번 살펴봅시다.
...
- 사용자가 버튼을 클릭합니다.  
- 버튼에 연결된 **이벤트 핸들러 함수**가 실행됩니다.  
- 이벤트 핸들러 안에서 **setState**를 호출하여 **State 값을 변경**합니다.  
- State가 변경되면 React가 자동으로 **컴포넌트를 다시 렌더링**합니다.  
- 화면에 최신 State 값이 반영됩니다.
...
### ✅ 핵심 요약
> 1. React 이벤트는 **camelCase**를 사용한다 \`onClick\`
> 2. 이벤트 핸들러에는 **함수 이름**만 전달한다. \`onClick={handleClick}\`
> 3. 상황에 따라서 **화살표 함수(Arrow function)** 를 사용하여 인자를 전달하거나 간단한 로직을 작성할 수 있다.
> 4. 이벤트를 통해 **State를 변경**하면 화면이 다시 그려진다.
...
> 1️⃣ **코드 완성** : 내 컴퓨터(Local)에서 리액트 앱 개발을 마칩니다.
...
1. **복잡한 서버 설정 제로** : 서버 설정이나 보안 인증서 같은 전문 지식이 없어도 괜찮습니다. 클릭 몇 번과 간단한 설정만으로 리액트 앱을 즉시 배포할 수 있습니다.
...
리액트 중급 레벨로 도약하기 위해 반드시 넘어야 할 산, 바로 컴포넌트의 인생을 다루는 **라이프 사이클(Life Cycle)** 과 마법 같은 **useEffect** 훅에 대해 다룰 예정입니다.

더 강력한 리액트 개발자가 될 준비가 되셨나요? 보너스 강의에서 다시 만나요! 🚀
...
지금까지 우리는 리액트로 화면을 만들고 데이터를 관리하는 법을 배웠습니다. 하지만 아직 우리 앱은 2% 부족합니다. 새로고침을 하면 데이터가 사라지고, 외부 세상(서버나 브라우저 저장소)과 소통하는 법을 모르기 때문이죠.

이번 섹션에서는 리액트 중급 레벨로 올라가기 위한 필수 관문인 **라이프사이클** 과 **useEffect** , 그리고 **스토리지** 에 대해 배웁니다.
...
> - **useEffect 훅** : "원하는 타이밍에 코드를 실행"하는 리액트의 핵심 마법을 배웁니다.
...
실제 서비스에서 서버로부터 데이터를 불러오거나 로그인 상태를 유지하는 모든 기술의 기초가 바로 이곳에 담겨 있습니다. 자, 더 강력해진 리액트의 세계로 들어가 볼까요?`,
    },
    {
...
* **Angular**：Googleが開発した、Web開発に必要な機能が一通りそろったフレームワークです。
* **Spring**：Java開発者が、決められたルールに従ってサーバーを作るときに使用します。
* **Django**：PythonでWebサービスを作る際、決められた構造に沿って開発を行うフレームワークです。
...
Reactは**SPA(Single Page Application)** 方式で動作します。画面全体が「パッ」と切り替わることなく、内容がスムーズに変わる秘密がまさにここにあります。
...
**SPA**は最初に、まるで空のキャンバスのような**index.html**を1つだけ受け取ります。その後、ユーザーがメニューをクリックすると、新しいページを読み込む代わりに、JavaScript が既存の画面の部品を取り外し、**新しい部品(コンポーネント)だけ**をその場所に差し込みます。
 
**2. アドレスバーのトリック(Routing)**
...
- **重複ロードの削減** : ロゴ、メニューバー、背景など変わらない部分はそのままにして、**変える必要がある部分だけ**を更新するからです。
 
- **データ中心の通信** : 重いHTMLのかたまりを毎回受け取るのではなく、とても軽い**データ(JSON)** だけをやり取りするため、通信速度が圧倒的に速くなります。
...
1. **バージョン確認**：ターミナル(またはCMD)を開き、次のように入力してください。\`node -v\`
2. **推奨バージョン**：**Vite**を使用するには**Node.js 18.0.0**以上のバージョンが必要です。(安定した学習環境のため、最新の LTS バージョンのインストールをおすすめします。)
3. **インストール方法**：もしコマンドが認識されない場合は、[Node.js公式サイト](https://nodejs.org/)からインストールしてください。
...
4️⃣ **不要なコードとファイルの削除**
> 1. **index.css**：ファイル内のすべてのコードを選択して削除(空の状態)にしてください。
> 2. **App.css**：このファイルは使用しないため、**ファイル自体を削除**します。
> 3. **App.jsx**：以下のコードだけを残し、それ以外はすべて削除してください。
...
**JSX**はHTMLと似た見た目をしていますが、実体はJavaScriptであるため、いくつかの厳格なルールがあります。
...
JavaScriptにおいて**class**は予約語(特別な意味を持つ単語)として既に使用されています。そのため、CSSクラスを指定する際は**className**を使用します。
...
HTMLでは閉じなくても許容されていた上記のようなタグも、JSXでは必ず
\`\`\`jsx
<img />
<input />
\`\`\`
のように **自己完結型(Self-closing)** にするか、閉じタグを書かなければなりません。`,
  },
  {
    id: "section2-quiz-1",
    section: 2,
  ...
  },
  {
    ...
    exp: 5,
    time: 3,
    content: `# 🏁 セクション 2 まとめ： コンポーネントと JSX
...
### ✅ 重要なポイント
- **コンポーネント**はUIの最小単位であり、名前は必ず**大文字**から始める。
- **JSX**はJavaScriptとHTMLが融合したような構文である。
...
Reactにおいて**State(ステート)** とは、 コンポーネントが内部的に**保持している値**のことです。 ユーザーとのやり取りに応じて、 いつでも**変更される可能性のあるデータ**を指します。
...
普通の変数は、値が変わっても**Reactが画面を更新すべきであるということに気付くことができません。** つまり、コンポーネントを **再描画(レンダリング)** しないため、画面上の数字は古いままになってしまいます。

---

### ✅ だからこそStateが必要です

Stateは単なるデータではなく、**「値が変わったから画面を再描画して！」** とReactに送る合図のようなものです。

> 📌 **ここで言う「レンダリング(Rendering)」とは？**
> 
> コンポーネント関数が再び呼び出され、その結果として変更されたデータが反映された**新しい画面(UI)がブラウザに描画されるプロセス**を指します。

Stateが変更されると、Reactは自動的にこのレンダリングプロセスを実行し、ユーザーが常に最新の値を見られるように制御してくれます。`,
  },
  {
    id: "what-is-usestate",
    section: 3,
...
コンポーネントの状態(State)を管理するためには、Reactが提供する**useState**という「フック(Hook)」を使用します。
この道具は、Reactアプリにおいて最も基本的かつ強力な役割を担っています。
...
> **💡 配列の分割代入とは？**
> 配列の中にある値を取り出して、変数に一度に代入する方法です。
> この方法の最大のメリットは、**自分の好きな名前を自由に付けられる**ことです。
\`\`\`jsx
const [a, b] = ['りんご', 'バナナ'];
 // a = 'りんご'
 // b = 'バナナ'
...
React はデータだけを渡すのではなく、そのデータを**安全に変更するための専用の鍵(更新関数)** を一緒に渡します。
データを直接書き換えるのではなく、この「鍵」を通してのみ修正を依頼することで、Reactは **「あ、データが変わったな。画面を更新しよう」** と正しく判断できるのです。`,
  },
  {
    id: "how-to-use-usestate",
    section: 3,
...
実際には、更新関数の名前を何にしてもReact自体は動作します。しかし、Reactコミュニティには世界共通の**約束事(慣習)** があります。
...
### 2. 直接書き換えの禁止(不変性の維持)
最も重要なルールです。状態の値は、必ず**状態更新関数**を通してのみ変更してください。値を直接代入して書き換えると、Reactは画面を更新すべきタイミングを見失ってしまいます。
...
### 3. 非同期な更新：「注文予約システム」
多くの初心者が戸惑うポイントです。状態を変更する関数を実行した直後に状態の値を確認してみると、まだ変更されていない以前の値が表示されます。
\`\`\`jsx
const [count, setCount] = useState(0);
...
**💡 なぜそうなるのでしょうか？(レストランの注文の例え)**
Reactの状態変更は、**「即時実行」ではなく、「注文予約」** のようなものです。

> 1. **setCount**を呼び出すのは、Reactに「次回、数を1つ増やしてください」と**注文書**を渡すことにあたります。
> 2. 注文書を渡したからといって、今自分が持っている**count**変数自体がすぐに変わるわけではありません。
> 3. Reactが注文を処理し、**画面を再描画(再レンダリング)**する際に、はじめて新しい値が**count**に反映されます。

---

### 💡(応用編)では、なぜ関数型アップデートを使うのでしょうか？
「関数型アップデート **(prev => prev + 1)** を使っても、**console.log**には同じく以前の値が出るのはなぜ？」と思うかもしれません。

関数型アップデートの本当の目的は、**「連続する注文」** を正確に処理することにあります。
つまり、複数回の状態変更が連続して発生しても、必ず最新の状態を基に正しく計算できるようにするためです。
\`\`\`jsx
// ❌ 通常のアップデート： 1 しか増えません
...
> **ログ確認用ではなく、あくまで「データの信頼性」のために使う**、という点を必ず覚えておきましょう。`,
  },
  {
    id: "counter-app-practice",
    section: 3,
...
まず最初に、Reactに状態を使うと知らせる**useState**をインポートします。フックは必ずコンポーネントの**最上部**で呼び出す必要があることを忘れないでください。
\`\`\`jsx
import { useState } from 'react';
...
_ブラウザで数字がリアルタイムに変わりますか？
おめでとうございます！これであなたの初めてのReact状態管理アプリが完成です。✨_
...
const [val, setVal] = useState(0);`,
  ...
  },
  {
    ...
    exp: 15,
    time: 3,
    content: `# 🏁 セクション 3 のまとめ： State と useState

おめでとうございます！Reactで最も重要な概念の一つ、**State(状態)** の壁を越えました。
今回のセクションでは、単に画面を描くだけでなく、自分で考え変化するアプリを作る方法を学びました。

---

### 📝 何を学びましたか？

- **State(状態)の概念** : コンポーネント内部で変化するデータを記憶する「メモリ装置」であることを理解しました。
- **useStateフック** : 状態値を安全に管理し、変更するためには必ず専用関数 **(setter)** を使う必要があることを学びました。
- **実習 - カウンターアプリ** : ボタンクリックイベントで状態を更新し、数字がリアルタイムで変わるReactの反応性を体験しました。

---

### 🎁 次のステップは？

これで私たちのコンポーネントはデータを覚えることができるようになりました。しかし、実際のアプリでは多数のコンポーネントが互いに会話し、データをやり取りします。

> _自分が持っている**State**を隣のコンポーネントに渡すにはどうすればいいですか？_

その答えは、次のセクション**Props**にあります。React のもう一つの重要な柱を学びに行きましょう！`,
  },
  // Section 4
  {
    id: "props-passing-data",
...
> **💡 値を変更したい場合は？**
> 直接変更するのではなく、前の講義で学んだように、親が渡してくれた **変更用の関数(setter)** を呼び出して「値を変更してください」と親に依頼する必要があります。
...
### ✅ 重要なポイント
> 1. **Props**は親から子へ渡される**データ**である。
> 2. 子コンポーネントはPropsを**変更できない。**(読み取り専用)
> 3. **関数**もPropsとして渡すことができ、それによって子が親の状態を操作できる。
> 4. _**{ text }**_ のような**分割代入**を使うとコードがスッキリする。
...
ユーザーのクリックや入力に反応する魔法、次のセクション**イベント(Event)** でお会いしましょう。 ⚡`,
  ...
  // Section 5
  {
    ...
    exp: 15,
    time: 5,
    content: `# ⚡ ユーザーとのやり取り、イベント(Event)

Reactにおける**イベント**とは、ユーザーが画面とやり取りする際に発生するあらゆる行動を指します。

![event](/assets/images/contents/event.png)

### ❓ なぜイベントが重要なのでしょうか？

ユーザーがボタンを押したり文字を入力したときに、画面を反応させるためには必ずイベントを受け取る必要があります。

\`\`\`jsx
// event flow
ユーザーの行動 → イベント発生 → 関数実行 → State変更 → 画面更新
\`\`\`
...
- ReactのイベントはHTMLイベントと似ていますが、**キャメルケース(camelCase)** を使用します。
  \`onclick → onClick, onchange → onChange, onsubmit → onSubmit\`
...
`,
  },
  {
    ...
    exp: 15,
    time: 5,
    content: `# 🆚 HTML vs React イベントの違い

Reactのイベントの書き方はHTMLと似ているように見えますが、覚えるべきポイントはたった**2つ**です。**名前**と**渡し方**です。

---

### 1️⃣ 表記方法の違い(CamelCase)

HTMLではすべてのイベントを小文字で書きますが、ReactではJavaScriptの慣習に従い**キャメルケース(camelCase)** で書く必要があります。
...
HTMLでは実行するコードを **「文字列」** として渡しますが、Reactでは実行する**関数名(参照)** を **{中括弧}** の中に直接書きます。

\`\`\`jsx
// ❌ HTML方式：関数を呼び出す「文」を文字列として渡す
<button onclick="handleClick()">クリック</button>

// ✅ React方式：関数「そのもの」を中括弧で渡す
<button onClick={handleClick}>クリック</button>
\`\`\`

> **⚠️ 注意：中括弧の中で関数を呼び出さないでください**
> \`onClick={handleClick()}\` のように後ろに括弧を付けると、ボタンを押す前に関数がすぐ実行されてしまいます。必ず**関数名だけ**を渡すようにしましょう。
`,
  },
  {
    id: "event-handler-function",
    ...
    exp: 20,
    time: 7,
    content: `# 🛠️ イベントハンドラー(Event Handler)
### 📝 イベントを処理する関数 = イベントハンドラー
> ユーザーがボタンをクリックしたり、文字を入力したりするような出来事を**イベント(Event)** と呼びます。
そして、その出来事が発生したときに React が何をすべきかを指示する関数を**イベントハンドラー(Event Handler)** と呼びます。
> 文字通り、発生した出来事を**処理(Handle)** する役割を持つ関数です。
...
### 💡 関数名の付け方
イベントハンドラーの名前は、次のように**どんな動作をするのか**が分かるように付けるのが一般的です。
...
`,
  },
  {
    ...
    exp: 20,
    time: 12,
    content: `# 🏹 簡潔な関数構文、アロー関数(Arrow Function)
Reactでイベントを扱うとき、**アロー関数(Arrow Function)** は非常によく使われる文法です。
コードをより **簡潔に書くことができ**、特にイベントで**引数を渡す**ときや**簡単なロジックを書く**ときに便利です。

![arrow function](/assets/images/contents/arrow_function.png)

---

### ❓ 名前付き関数 vs アロー関数、何が違うの？

結論から言うと、**どちらも最終的には関数です。**

コンポーネントの上部で名前を付けて定義する場合でも、イベントの位置でアロー関数として書く場合でも、**ボタンに実行する関数をつなげる**という点は同じです。

ただし、**使う場面が少し異なります。**

1. **名前付き関数(定義方式)**
   ロジックが長い場合や、複数の場所で**再利用**するときに有利です。

2. **アロー関数(その場で書く方式)**
   ロジックが短い場合や、特定の**引数(値)** をすぐに渡したいときに簡潔に書けます。

---

### 🚀 アロー関数が特に役立つ場面

#### 1️⃣ 引数が必要な関数を実行するとき(重要！)

特定のIDや値を関数に渡す必要がある場合、次のように書くと**クリック時ではなく即時実行**されてしまいます。

\`\`\`jsx id="1p6p8y"
// ❌ 間違った方法：コンポーネントがレンダリングされた瞬間に実行される
<button onClick={handleDelete(1)}>削除</button>
\`\`\`

このときアロー関数を使えば、**「クリックされたときに実行する関数」** を渡すことができます。

\`\`\`jsx id="s3saj0"
// ✅ 正しい方法：クリックしたタイミングで実行される
<button onClick={() => handleDelete(1)}>削除</button>
\`\`\`

> つまり、アロー関数は「今すぐ実行する」のではなく **「クリック時に実行する」** 関数を渡す役割をします。

---

#### 2️⃣ 1つのイベントで複数の処理を行うとき

別の関数を作らなくても、イベントの位置でそのまま複数行のコードを実行できます。
...
ロジックがシンプルな場合は、このように書く方が**コードが読みやすくなることも多い**です。
...
* コードが長く複雑な場合
  👉 上部に**名前付き関数**として整理するのがおすすめです。

* 引数を渡す場合や短いロジックの場合
  👉 **アロー関数**を使うと簡潔に書けます。

実際のReactコードでは、この2つの方法を**状況に応じて併用することが多い**です。
`,
  },
  {
    id: "counter-app-recap",
...
イベントの本当の目的は、ユーザーの入力を受けて**State**を変更することです。

皆さんはすでに**Section 3 - Stateカウンター実習**の講義で、クリックイベントを体験したことがあります。

今回の講義では、その記憶を振り返りながら、イベントが発生したときにStateがどのように変わり、その結果画面がどう更新されるのかを再確認してみましょう。
...
- ユーザーがボタンをクリックします。
- ボタンに紐づいた **イベントハンドラー関数** が実行されます。
- イベントハンドラー内で **setState** を呼び出し、**Stateの値を変更**します。
- Stateが変わると、Reactが自動で **コンポーネントを再レンダリング** します。
- 画面に最新の State 値が反映されます。

> つまり、**ユーザーの行動 → イベント → State変更 → 画面更新** という流れで繋がります。
> この流れを理解しておくと、後で複雑なイベントロジックでも State の変化を予測できるようになります。
...
これで、ユーザーのクリックに反応する生き生きとしたコンポーネントを作れるようになりました！

---

### ✅ 重要ポイントまとめ
> 1. Reactのイベントは**camelCase**を使う。 \`onClick\`
> 2. イベントハンドラーには**関数名**だけを渡す。 \`onClick={handleClick}\`
> 3. 場合によって**アロー関数(Arrow function)** を使い、引数を渡したり簡単なロジックを書くことができる。
> 4. イベントを通じて**Stateを変更**すると、画面が再レンダリングされる。

---

次のセクションでは、複数のデータを一度に扱う方法と**不変性（Immutable）** について学びます。

いよいよ本格的にデータ操作の方法を学ぶ時間です！ 🚀
`,
  },
  // Section 6
  {
